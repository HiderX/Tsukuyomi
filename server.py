...
# 回复要求
用一两句话简短、自然地回复彩叶，语气符合八千代人设（温和、略带从容）。
【重要】每条回复的末尾必须换行后单独写一行动作代码，格式为：ACTION:动作名。当用户明确要求你做某个动作时（如「笑一个」「打个招呼」「害羞一下」「难过一下」「哭一下」等），回复可以保持人设，但动作代码必须与用户要求一致，这样立绘才会真的做出对应动作。动作名从下面选（只写英文）：
- idle：平静、陈述、中性、一般说明时
- greet：开心、打招呼、欢迎、道别时；用户说「打招呼」「挥挥手」「嗨一下」等时必用 ACTION:greet
...
- cry：【强制】用户消息里出现「哭」「哭一个」「哭一下」「让你哭」「我让你哭一下」「掉眼泪」「流泪」等任一表述时，必须在该条回复末尾换行写 ACTION:cry，否则立绘不会播哭脸。
示例：彩叶，晚上好呀～今天在月读空间里玩得开心吗？
ACTION:greet"""
...
def create_app():
    ...
    def _fetch_status_page():
        status_url = os.environ.get("STATUS_PAGE_URL", "https://status.qkv.io/status/default")
        try:
            ...
            return ("【月读空间/网站运行状态】以下为状态页当前内容，请据此回答用户关于运行状态、宕机、服务可用性的问题。\n\n" + text) if text else ""
        except Exception:
            ...
            if choices and isinstance(choices, list):
                ...
                if msg and isinstance(msg, dict):
                    ...
                    if isinstance(raw, str):
                        display_text, action = _parse_action_and_strip(raw)
                        msg["content"] = display_text
                        if _cry_trigger.search(last_user) and action not in ("cry", "tearful"):
                            if DEBUG:
                                print(f"[chat] 用户消息含哭相关，强制 action=cry (原 action={action})")
                            action = "cry"
                        resp_data["live2d_action"] = action
                        if DEBUG:
                            print(f"[chat] 解析 action={action} display_len={len(display_text)}")
